...
def filteringdata_version2(request):
    ...
    if collegename=="bsit":
        ...
        first_list = ['collegename','year','ece','eee','cse']
        # @tablecreationdata
        data=tablecreationdata(request,head_list=first_list,first_list=newlist)
        # div_root = ET.Element('div')
        # tab = ET.SubElement(div_root, 'table', clas='table',border='1')
        # thr_root = ET.SubElement(tab, 'tr')
        ...
        # for index1,j in enumerate(newlist):
        #     print j,"LLJJJJJ"
        #     print index1
        #     tr_root = ET.SubElement(tab, 'tr')
        #     for number,k in enumerate(j):                
        #         tad = ET.SubElement(tr_root,'td', id='{0}?{1}?{2}'.format(collegename,yearsdict[collegename][index1],first_list[number])).text=str(k)                
        #     # break 
        # data = ET.tostring(div_root).replace('clas','class')

    return render(request,'static_data_version_2.html',{'listdata':firstpartadd, 'table_data':data, 'colleges':list(set(college_names_list)), 'years':list(set(yelist))})
    ...
    # ye_dic={}
    # for i in ye:
    #     yelist=str(i.subject).split(",")
    #     print yelist,"++++++++++++++++"

    # adddat=[]
    # ecedat=subject.objects.filter(collegename="bsit",branch="ece",year="2016")
    # ece=0
    # eee=0
    # cse=0
    # eeedat=subject.objects.filter(collegename="bsit",branch="eee",year="2016")
    # csedat=subject.objects.filter(collegename="bsit",branch="cse",year="2016")
    # adddat.append(len(ecedat))
    # adddat.append(len(eeedat))
    # adddat.append(len(csedat))
    # print adddat,"..............."
    # head_list = ['collegename','branch','year']
    # div_root = ET.Element('div')
    # tab = ET.SubElement(div_root, 'table', clas='table',border='1')
    # thr_root = ET.SubElement(tab, 'tr')
    # tr_root = ET.SubElement(tab, 'tr')
    ...
    # return render(request,'staticdatas.html',{'listdata':adddat, 'table_datas':data})
 
    #return HttpResponse('process is done')
...
def staticdata(request):
    ...
    # for i in var:
    #     result_list.append(i.projectname)
    #     result_list.append(i.episode)
    ...
    # print ">>>>",result_list
    # for i in var:
    #     order_dict = {'project':'','episode':'','assigned':0,'wip':0,'complete':0}
    #     order_dict['project']=i.projectname
    #     order_dict['episode']=i.episode
    #     if i.status=="assigned":
    #         a+=1
    #         order_dict['assigned']=a
    #     elif i.status=="WIP":
    #         w+=1
    #         order_dict['wip']=w
    #     elif i.status=="complete" :
    #         c+=1    
    #         order_dict['complete']=c
...
#     print a,c,w,"{{{{{{{{{{{{{"
        ...
        #print i.status,i.episode,i.shotno,"::::::::::::::::::"
    # projects_list = ['five_it','jbs3', 'miles3', 'pdt', 'mrr']

    # #for i in projects_list:
    #     #project.objects.create(project_name=i)
    # print "projectrs created successfully"
...
def tablecreationdata(request, head_list=[],first_list=[]):    
    div_root = ET.Element('div')
    tab = ET.SubElement(div_root, 'table', clas='table',border='1')
    thr_root = ET.SubElement(tab, 'tr')
    ...
    for index1,j in enumerate(first_list):
        tr_root = ET.SubElement(tab, 'tr')
        for number,k in enumerate(j):                
            tad = ET.SubElement(tr_root,'td', id='').text=str(k)                     
    data = ET.tostring(div_root).replace('clas','class')
    return data
